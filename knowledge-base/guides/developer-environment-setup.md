...
Add the following three entries to your VS Code **workspace settings** (in the `.code-workspace` file generated per repo, or in `.vscode/settings.json`). They use a relative path that works because `obsidian\` sits at the same level as your repos — `..` goes up from the repo to your root, then into the vault.
...
Replace `{your-internal-path}` with the folder path inside your vault that leads to `knowledge-base`. This is whatever you named your vault folders in Obsidian.
...
These three settings enable:

| Setting | What it registers |
|---|---|
| `chat.agentFilesLocations` | Shared agents in the VS Code agent picker |
| `chat.instructionsFilesLocations` | Shared instructions for code generation and review |
| `chat.promptFilesLocations` | Shared slash commands (`/init-codespace`, `/create-feature`, etc.) |
