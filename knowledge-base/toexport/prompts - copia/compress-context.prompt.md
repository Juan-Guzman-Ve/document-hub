---
mode: 'agent'
description: 'Compress and persist the current feature context to context/context-latest.md'
---
...
If no matching folder is found, stop and tell the human — the feature folder may not have been created yet (`/create-feature` must be run first).
...
3. If `context-latest.md` already exists inside it → rename it to `context-{YYYY-MM-DD-HHmm}.md` using the current timestamp before proceeding

---

## Step 3 — Build the snapshot

Produce the contents of the new `context-latest.md` following the template defined in `context-compression.md`.
...
Present the full draft of `context-latest.md` to the human before writing.
...
1. Write `context-latest.md` to `{feature-folder}/{feature-name}-AB#{number}/context/context-latest.md`
2. Confirm the file was saved and show the full path
3. If an old snapshot was renamed in Step 2, confirm that as well

Tell the human the context is saved and the session can be safely closed. When work resumes, the next agent will read `context-latest.md` to restore state instantly.
